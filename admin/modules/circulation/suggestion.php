...
/**
 * @Author: ido
 * @Date:   2016-10-09 21:33:41
 * @Last Modified by:   ido
 * @Last Modified time: 2016-10-09 21:40:54
 */
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
  ...
  $data['type'] = trim($dbs->escape_string(strip_tags($_POST['type'])));
  $data['sug_desc'] = trim($dbs->escape_string(strip_tags($_POST['desc'])));
  ...
  if (isset($_POST['updateRecordID'])) {
    ...
    // update the data
    $update = $sql_op->update('suggestion', $data, 'sug_id='.$updateRecordID);
    if ($update) {
...
<div class="menuBoxInner masterFileIcon">
  <div class="per_title">
      <h2><?php echo __('Suggestion List'); ?></h2>
  </div>
  <div class="sub_section">
    <div class="btn-group">
    </div>
    <form name="search" action="<?php echo MWB; ?>circulation/suggestion.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
    <input type="text" name="keywords" size="30" />
    <input type="submit" id="doSearch" value="<?php echo __('Search'); ?>" class="button" />
    </form>

    <div class="btn-group">
        <a href="<?php echo MWB; ?>circulation/suggestion.php" class="btn btn-default"><i class="glyphicon glyphicon-list-alt"></i>&nbsp;<?php echo __('Suggestion List'); ?></a>
        <a href="<?php echo MWB; ?>circulation/suggestion.php?needreview=true" class="btn btn-default" style="color: #FF0000;"><i class="glyphicon glyphicon-list-alt"></i>&nbsp;<?php echo __('Suggestion need review'); ?></a>
    </div>
  </div>
</div>
</fieldset>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  $rec_q = $dbs->query('SELECT * FROM suggestion WHERE sug_id='.$itemID);
  ...
  if ($rec_q->num_rows > 0) {
      ...
      // type
      $form->addTextField('text', 'type', __('Type'), $rec_d['type'], 'style="width: 50%;"');
      // status
      ...
      // description
      $form->addTextField('textarea', 'desc', __('Description'), $rec_d['sug_desc'], 'rows="5" style="width: 100%; overflow: auto;"', '');
      echo $form->printOut();
  }
} else {
  // create datagrid
  ...
  // table spec
  $table_spec = 'suggestion AS r';
  $datagrid = new simbio_datagrid();
  if ($can_read AND $can_write) {
    $datagrid->setSQLColumn('r.sug_id',
      'r.name AS \''.__('Name').'\'',
      'r.email AS \''.__('Email').'\'',
      'r.type AS \''.__('Type').'\'',
